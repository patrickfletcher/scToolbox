function [P, adj_P, flag] = diptest(X, minN, nboot, correctionmethod, doPlot, genes)

if ~exist('minN','var') || isempty(minN), minN=4; end
if ~exist('nboot','var') || isempty(nboot), nboot=100; end
if ~exist('correctionmethod','var') || isempty(correctionmethod), correctionmethod='fdr'; end
if ~exist('doPlot','var') || isempty(doPlot), doPlot=false; end

if minN<4, minN=4; end

% dip=ones(size(X,1),1);
P=ones(size(X,1),1);
xlow=ones(size(X,1),1);
xup=ones(size(X,1),1);
flag(size(X,1),1)="";
...
for i=1:length(P)
    ...
    if ~isempty(x) && length(x)>minN && max(x)>min(x)
        [~, P(i), xlow(i), xup(i)] = hartigansdipsigniftest(x,nboot);
        
%         doPlot=false;
        if doPlot && P(i)<0.01
            histogram(x,ceil(sqrt(numel(x))))
            ax=gca;
            line(xlow(i)*[1;1], ax.YLim'*ones(1,2),'color','b')
            line(xup(i)*[1;1], ax.YLim'*ones(1,2),'color','r')
...
%vectorized version
unifpdfboot=sort(unifrnd(0,1,nboot,N));
boot_dip=Inf(nboot,1);
for i = 1:nboot
    boot_dip(i)=hartigansdiptest(unifpdfboot(i,:));
end
...
%NOTES: four for loops eat up all the computation time. Vectorize????
