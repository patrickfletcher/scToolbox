function [C,eigenvals, A]=diffuse(X,options)
arguments
    X
    options.metric='cosine'
    options.eps_val=0.05
    options.ndc=[]
    options.t=[]
    options.plotit=0
end
...
D=squareform(pdist(X',options.metric));
% pw1=fileparts(which(mfilename));
% addpath(fullfile(pw1,'thirdparty\DiffusionMap\'));
[C, eigenvals, A] = i_diffuse(D,options.eps_val,options.ndc,options.t);
if options.plotit
    scatter(C(:,1),C(:,2));
end
end

function [X, eigenvals, A, psi, phi] = i_diffuse(D,eps_val,neigen,t)
arguments
    D
    eps_val=0.05
    neigen=[]
...
if ~isempty(t) % fixed scale parameter
