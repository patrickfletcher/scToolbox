...
def estimate_profile_sgt(profile, nz_feat):
    """ Estimate a gene expression profile by Simple Good Turing.
    Args:
      profile (np.array(float)): summed counts per gene -> matrix[nz_feat, :].sum(axis=1)
      nz_feat (np.array(int)): Indices of features that are non-zero at least once
    Returns:
      profile_p (np.array(float)): Estimated probabilities of length len(nz_feat).
    """
